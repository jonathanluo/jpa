...
import org.moonwave.jpa.model.pojo.DeptManager;
 ...
 *
 */
public class DeptManagerNativeQuery 
{
...
//					 "e.last_name as lastName, dm.dept_no from employees e, dept_manager dm where e.emp_no = dm.emp_no order by e.first_name", DeptManager.class);
		Query query = em.createNativeQuery("select e.emp_no as employeeNO, e.birth_date, e.hire_date, e.first_name as FIRSTNAME, " + 
				 "e.last_name as LastNAME, dm.dept_no from employees e, dept_manager dm where e.emp_no = dm.emp_no order by dm.dept_no", DeptManager.class);
		query.setMaxResults(30);
		List<DeptManager> list = (List<DeptManager>) query.getResultList();
		int i = 0;
		for (Object emp : list) {
    ...
    {
    	DeptManagerNativeQuery test = new DeptManagerNativeQuery();
    	try {
    		test.setUp();
    		test.testQuery();
    		test.tearDown();
